    ...
    value: function buildTokenTree() {
      ...
      while (++this.i < ts.length) {
        if (isOpenToken(ts[this.i])) {
          ...
          new this.constructor(ts[this.i].children, this.options, this.rOptions, this.env).forEach(function (token) {
...
var Renderer = function () {
  ...
  createClass(Renderer, [{
    ...
    value: function render() {
      ...
      return this.renderTokenTree(this.buildTokenTree(tokens, remarkableOptions, env), env);
    }
  }, {
    key: 'buildTokenTree',
    value: function buildTokenTree(tokens, remarkableOptions, env) {
      return new TokenTree(tokens, this.options, remarkableOptions, env);
    }
  }, {
    key: 'renderTokenTree',
    value: function renderTokenTree(token, env) {
      var _this = this;

      var index = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;

      if (Array.isArray(token)) return token.map(function (token, indx) {
        return _this.renderTokenTree(token, env, indx);
      });
      if (!token || !token.type) return token;
      if (!this.options.components[token.type]) return null;

      var envProps = typeof this.options.components[token.type] === "string" ? {} : env;

      return React.createElement(this.options.components[token.type], assign({}, envProps, token.props, {
